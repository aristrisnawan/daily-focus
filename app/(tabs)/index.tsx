import { fetchHabits, updateHabitSatatus } from '@/api/habbit';
...
type Habit = { id: string; name: string; done: boolean; icon?: string; type: 'habit' };
...
export default function HomeScreen() {
  ...
  const { data: habitsData = [], isPending: isLoadingHabits, isError: isErrorHabits, error: errorHabits } = useQuery({
    queryKey: ['habits'],
    queryFn: fetchHabits
  })
  ...
  const updateHabitMutatoin = useMutation({
    mutationFn: updateHabitSatatus,
    onSuccess: () => {
  ...
  const handelToggleHabit = (habit: Habit) => {
    const updateHabit = { ...habit, done: !habit.done }
            ...
            if (item.type === 'habit') {
              return (
                <View style={[styles.checkboxStyle, { marginHorizontal: 16 }]}>
                  <Checkbox
                    value={item.done}
                    onValueChange={() => handelToggleHabit(item)}
                  />
                  <Text
                    style={{
                      marginLeft: 8,
